...
- `SharedResources.cs.resx` - Czech translations
...
2. Add the corresponding translation to `SharedResources.cs.resx` (Czech)
...
1. Create a new resource file: `SharedResources.[culture].resx` (e.g., `SharedResources.de.resx` for German)
...
Czech (cs) is set as the default language. The application will use Czech translations by default for all users. Users can potentially override this by setting the Accept-Language header in their browser, though no UI is currently provided for language switching.

## Technical Notes

- Resource files are compiled into satellite assemblies during build
- The Czech resource assembly is located at: `bin/Debug/net9.0/cs/EsonicModule.resources.dll`
